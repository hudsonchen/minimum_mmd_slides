...
\renewcommand{\citet}[1]{{\scriptsize\textcolor{graytext}{\oldcitep{#1}}}}
...
\begin{frame}{Overview}
    \begin{itemize}
        \item What is Maximum mean discrepancy (MMD)?
        \item Minimization of MMD (miniMMD)
    \end{itemize}
    \vspace{0.10cm}
    \begin{columns}[b,onlytextwidth]
        ...
        \fcolorbox{mmdBlue!70!black}{mmdBlue!8}{%
            \begin{minipage}[t][0.34\textheight][t]{0.88\linewidth}
                ...
                {\bfseries Sampling}\\[-0.02cm]
                {\bfseries generative modelling}\\[0.22cm]
    ...
    \vspace{0.3cm}
    \begin{itemize}
        \item Future work: MiniMMD for causal inference.
    \end{itemize}
\end{frame}
...
\begin{frame}{Background: Maximum Mean Discrepancy (MMD)~\citep{gretton2012kernel}}
    ...
    \end{columns}
    \begingroup
    \setbeamercolor{block title}{fg=black!25,bg=black!5}
    \setbeamercolor{block body}{fg=black!25,bg=black!2}
    \setbeamercolor{itemize item}{fg=black!25}
    \setbeamercolor{normal text}{fg=black!25}
    \hypersetup{citecolor=black!25}
    \color{black!25}
    \begin{thm*}
    ...
    \end{thm*}
    \begin{itemize}
        \item \textcolor{black!25}{MMD is a probability divergence ($\mathrm{MMD}(\Pb, \Qb)=0 \iff \Pb=\Qb$) if $k$ is characteristic (Gaussian)~{\scriptsize\oldcitep{sriperumbudur2011universality}}.}
    \end{itemize}
    \endgroup
\end{frame}
...
\begin{frame}{Part I: Numerical Integration}
    ...
    \color{black!25}
    \begin{itemize}
        \item \textcolor{black!25}{When $w_1=\cdots=w_n=1/n$, this is Monte Carlo. $\hat{I}_{MC} = \frac{1}{n}\sum_{i=1}^n f(x_i)$.}
        \begin{align*}
            \color{black!25}\E\left[|\hat{I}_{MC} - I|\right] = \mathcal{O}(n^{-1/2}).
        \end{align*}
        \item \textcolor{black!25}{Or equivalently, to obtain an $\epsilon$-accurate estimate, we need $n = \mathcal{O}(\epsilon^{-2})$ samples.}
    \end{itemize}
    \setbeamercolor{block title example}{fg=black!25,bg=black!5}
    \setbeamercolor{block body example}{fg=black!25,bg=black!2}
    \setbeamercolor{answer box}{fg=black!25,bg=black!2}
    \color{black!25}
    \begin{exampleblock}{}
        \centering
...
\begin{frame}{Part I: Numerical Integration}
    ...
    \color{black!25}
    \begin{itemize}
        \item \textcolor{black!25}{The right-hand side is the worst-case error of the quadrature rule.}
        \begin{align*}
            \color{black!25}w_{1:n} &\color{black!25}:= \arg\min_{w_{1:n}\in\R^n} \mmd\left(\sum_{i=1}^n w_i \delta_{x_i}, \Pb\right), \\ 
            \color{black!25}w_{1:n} &\color{black!25}= \E_{X \sim \Pb}[k(X, x_{1:n})]
            K(x_{1:n},x_{1:n})^{-1} . 
        \end{align*}
        \item \textcolor{black!25}{The kernel quadrature (KQ) rule~{\scriptsize\oldcitep{rasmussen2002bayesian,sommariva2006numerical,briol2019probabilistic}}: $\hat{I}_{\mathrm{KQ}} = \sum_{i=1}^n w_i f(x_i)$.}
    \end{itemize}
    \endgroup
\end{frame}
...
\begin{frame}{Part I: Numerical Integration}
    \begin{itemize}
        ...
        \begin{align*}
            ...
            K(x_{1:n},x_{1:n})^{-1} . 
        \end{align*}
        \item The kernel quadrature (KQ) rule: $\hat{I}_{\mathrm{KQ}} = \sum_{i=1}^n w_i f(x_i)$.~\citep{rasmussen2002bayesian,sommariva2006numerical,briol2019probabilistic}
    \end{itemize}
\end{frame}
...
\begin{frame}{Part I: Numerical Integration}
    ...
    \begin{columns}[T,onlytextwidth]
        ...
        \begin{minipage}{0.94\linewidth}
            ...
            \[
                ...
                =
                \sum_{t=1}^{T}
                w_t^{\Theta} f\left(\hat{J}_{\mathrm{KQ}}(\theta_t)\right)
            \]
            \vspace{-0.10cm}
        \end{minipage}}
    \end{columns}
    ...
    \begin{tabular}{@{}c@{\hspace{0.24cm}}c@{}}
        {\bfseries To achieve error $|I-\widehat{I}|\leq\Delta$:}
        &
        \resizebox{0.56\linewidth}{!}{%
        \begin{tabular}{@{}c@{\hspace{0.45cm}}c@{}}
            \toprule
            \textbf{NMC}~{\scriptsize\oldcitep{rainforth}}: $\Delta^{-4}$
            &
            \textbf{NQMC}~{\scriptsize\oldcitep{bartuska2023double}}: $\Delta^{-2.5}$
            \\
            \textbf{MLMC}~{\scriptsize\oldcitep{giles2015multilevel}}: $\Delta^{-2}$
            &
            \textbf{NKQ}:  $\Delta^{-d_{\calX}/s_{\calX}-d_{\Theta}/s_{\Theta}}$
            \\
            \bottomrule
        \end{tabular}}
    \end{tabular}
...
\begin{frame}{Part I: Numerical Integration}
    ...
    \begin{tabular}{@{}c@{\hspace{0.24cm}}c@{}}
        ...
        \begin{tabular}{@{}c@{\hspace{0.45cm}}c@{}}
            \toprule
            \textbf{NMC}~\citep{rainforth}: $\Delta^{-4}$ 
            &
            \textbf{NQMC}~\citep{bartuska2023double}: $\Delta^{-2.5}$
            \\
            \textbf{MLMC}~\citep{giles2015multilevel}: $\Delta^{-2}$
            &
            \textbf{NKQ}: $\Delta^{-d_{\calX}/s_{\calX}-d_{\Theta}/s_{\Theta}}$
            \\
            \bottomrule
...
\begin{frame}{Part II: Sampling / Generative modelling}
    ...
    \end{exampleblock}
    \begin{itemize}
        \item This problem can be written as an \blue{optimization} problem on $\calP(\R^d)$, the space of probability distributions~\citep{ambrosio2005gradient,villani2009optimal,santambrogio2017euclidean}:
        \begin{align*}
...
\begin{frame}{Part II: Sampling / Generative modelling}
    ...
    \end{talign*}
        \item A (Wasserstein) gradient descent of $\mmd^2(\Pb, \Qb)$ gives a sequence of distributions: $\Pb_0, \Pb_1, \ldots, \Pb_t$~\citep{arbel2019maximum,korba2021kernel}.
        \begin{itemize}
    ...
    \centering
    \begin{tikzpicture}
        \node[inner sep=0pt] (samplingpreview) {\includegraphics[width=0.85\linewidth]{../plotting/figures/illustration_sampling.pdf}};
        \fill[white, opacity=0.72] (samplingpreview.south west) rectangle (samplingpreview.north east);
    \end{tikzpicture}\\[0.12cm]
    \begin{itemize}
        \setbeamercolor{itemize item}{fg=black!25}
        \item \textcolor{black!25}{MMD flow generates more uniform samples.}
    \end{itemize}
    \endgroup
\end{frame}
...
\begin{frame}{Part II: Sampling / Generative modelling}
    ...
    \begin{itemize}
        \item MMD flow does not outperform diffusion-based generative models~\citep{galashov2024deep,altekruger2023neural,hertrich2024wasserstein,hertrich2023generative,hagemann2024posterior,belhadji2025weighted,belhadji2026discretize}.
...
\begin{frame}{Part II: Sampling / Generative modelling}
    ...
    \begin{itemize}
        ...
        \end{itemize}
        \item Other work on convergence of MMD flow~\citep{chizat2026quantitative,boufadene2023global}. 
    \end{itemize}
\end{frame}
...
\begin{frame}{Part II: Sampling / Generative modelling}
    ...
    \end{exampleblock}
    \centering \includegraphics[width=0.75\linewidth]{../plotting/figures/sampling_four_panel_summary.pdf}\\[0.12cm]
    \begin{itemize}
...
\begin{frame}{Part II: Sampling / Generative modelling}
    ...
    \color{black!25}
    \begin{itemize}
        \item \textcolor{black!25}{As $n\to\infty$, $\mathcal{G}_n$ approximates the whole RKHS $\calH$ up to constant functions.}
        \item \textcolor{black!25}{Stationary MMD points achieve $o(n^{-1/2})$ rate for numerical integration.}
    \end{itemize}
    \endgroup
\end{frame}
...
\begin{frame}{Part III: Statistical inference}
    ...
    \begin{itemize}
        \item Applications: \citet{key2025composite,schrab2023mmd,bruck2025distribution,fazeli2023semi,alquier2024universal,alquier2026timeseries,dellaporta2025decision,dellaporta2022robust,dellaporta2023robust,chen2025total,su2025differentiable,alquier2023estimation,cherief2025parametric,khoo2026minimum}.
    \end{itemize}
...
\begin{frame}{Part III: Statistical inference}
    ...
    \begin{minipage}[t][0.29\textheight][t]{0.95\linewidth}
        \centering
        {\bfseries A Gradient Flow Perspective of Minimum MMD Estimation~\textcolor{mmdBlue!80!black}{(Arxiv)}}\\[-0.10cm]
