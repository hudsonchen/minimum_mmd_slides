...
\begin{frame}{Background: Maximum Mean Discrepancy (MMD)~\citep{gretton2012kernel}}
    ...
    \end{columns}
    \begingroup
    \setbeamercolor{block title}{fg=black!35,bg=black!8}
    \setbeamercolor{block body}{fg=black!35,bg=black!3}
    \setbeamercolor{itemize item}{fg=black!35}
    ...
    \end{thm*}
    \begin{itemize}
        \item \textcolor{black!35}{MMD is a probability divergence ($\mathrm{MMD}(\Pb, \Qb)=0 \iff \Pb=\Qb$) if $k$ is characteristic (Gaussian)~\citep{sriperumbudur2011universality}.}
    \end{itemize}
    \endgroup
\end{frame}
...
\begin{frame}{Part I: Numerical Integration}
    ...
    \color{black!35}
    \begin{itemize}
        \item \textcolor{black!35}{The right-hand side is the worst-case error of the quadrature rule.}
        \begin{align*}
            \color{black!35}w_{1:n} &\color{black!35}:= \arg\min_{w_{1:n}\in\R^n} \mmd\left(\sum_{i=1}^n w_i \delta_{x_i}, \Pb\right), \\ 
            \color{black!35}w_{1:n} &\color{black!35}= \E_{X \sim \Pb}[k(X, x_{1:n})]
            K(x_{1:n},x_{1:n})^{-1} . 
        \end{align*}
        \item \textcolor{black!35}{The kernel quadrature (KQ) rule: $\hat{I}_{\mathrm{KQ}} = \sum_{i=1}^n w_i f(x_i)$.}
    \end{itemize}
    \endgroup
\end{frame}
